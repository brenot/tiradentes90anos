...
class CED_CAF_Core_Class
{
	public function __construct()
	{
		...
		$this->ced_caf_add_hooks_and_filters_to_be_used();
	}
	...
	function ced_caf_include_plugin_files_to_be_used()
	{
		...
		/**
		 * This function adds necessary hooks and filters.
		 * @name ced_caf_add_hooks_and_filters_to_be_used()
		 * @author CedCommerce <plugins@cedcommerce.com>
		 * @link: http://cedcommerce.com/
		 */
	
	function ced_caf_add_hooks_and_filters_to_be_used()
	{
	...
	function ced_caf_add_plugin_row_meta($plugin_meta, $plugin_file, $plugin_data, $status)
	{
		...
		{
			$plugin_meta[] = '<a href="http://demo.cedcommerce.com/woocommerce/ajaxify-filter/wp-admin" target="_blank">' . __( 'Demo : Backend', 'caf_txt_domain' ) . '</a>';
			$plugin_meta[] = '<a href="http://demo.cedcommerce.com/woocommerce/ajaxify-filter/" target="_blank">' . __( 'Demo : Frontend', 'caf_txt_domain' ) . '</a>';
			$plugin_meta[] = '<a href="http://demo.cedcommerce.com/woocommerce/ajaxify-filter/doc/index.html" target="_blank">' . __( 'Plugin Documentation', 'caf_txt_domain' ) . '</a>';
	...
	*/
	...
	function ced_caf_custom_script_enqueue_function()
	{
		...
		wp_enqueue_script('pro_cat_script', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/js/pro-cat-script.js',array('jquery'),'',true);
		wp_enqueue_style ( 'pro_cat', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/css/pro_cat.css');

		wp_enqueue_script('jquery_ui_js', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/js/price-slider/jquery_ui.js',array('jquery'),'',true);
		wp_enqueue_style ( 'jquery_ui_css', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/css/price-slider/jquery_ui.css');
		
		wp_enqueue_script('searchProduct', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/js/searchProduct.js',array('jquery'),'',true);
		
		wp_localize_script( 'searchProduct', 'searchProduct_ajax', array(
				'ajax_url' => admin_url( 'admin-ajax.php' )
		));

		wp_enqueue_script('priceWidgetScript', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/js/priceWidgetScript.js',array('jquery'),'',true);

		
		wp_localize_script( 'pro_cat_script', 'pro_cat_script_ajax', array(
			'ajax_url' => admin_url( 'admin-ajax.php' )
		));
	...
	function ced_caf_admin_script_enqueue_function()
	{
		...
		if(!(strpos($_SERVER['REQUEST_URI'], "wp-admin/widgets.php")))
		{
			return;
		}

		wp_enqueue_script('widget_script', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/js/widget_script.js',array('jquery'),'',true);
		
		wp_localize_script( 'widget_script', 'widget_script_ajax', array(
				'ajax_url' => admin_url( 'admin-ajax.php' )
		));

		wp_enqueue_style ( 'colorpicker_css', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/colorpicker/css/colorpicker.css');
		
		wp_enqueue_style ( 'extra_added', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/colorpicker/extra_added.css');

		wp_enqueue_script('colorpicker_js', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/colorpicker/js/colorpicker.js',array('jquery'),'',true);

		wp_enqueue_script('colorPickerDynamic', plugins_url().'/'.CED_CAF_PLUGIN_NAME.'/js/colorPickerDynamic.js',array('jquery'),'',true);
