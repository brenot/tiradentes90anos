...
class CED_CAF_Widget_Pro_Categories extends CED_CAF_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Show list of categories in widget and lets you search products based on category.', 'caf_txt_domain' );
		$this->widget_id          = 'ccas_ajax_product_categories';
		$this->widget_name        = __( 'AJAX Product Categories', 'caf_txt_domain' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Product Categories', 'caf_txt_domain' ),
				'label' => __( 'Title', 'caf_txt_domain' )
			),
			'orderby' => array(
				'type'  => 'select',
				'std'   => 'name',
				'label' => __( 'Order by', 'caf_txt_domain' ),
				'options' => array(
					'order' => __( 'Category Order', 'caf_txt_domain' ),
					'name'  => __( 'Name', 'caf_txt_domain' )
				)
			),
			'count' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Show product counts', 'caf_txt_domain' )
			),
			'hierarchical' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => __( 'Show hierarchy', 'caf_txt_domain' )
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Only show children of the current category', 'caf_txt_domain' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		require_once( CED_CAF_PLUGIN_DIR_PATH. '/core/walkers/class-ced-caf-pro-cat-walker.php' );
			...
			$list_args['show_option_none']           = __('No product categories exist.', 'caf_txt_domain' );
