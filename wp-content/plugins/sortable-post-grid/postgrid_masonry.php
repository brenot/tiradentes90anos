<?php
function postgrid_masonry($atts)
{
    global $post;
...
//			}
    ...
    $html .= '<div id="loop-content" class="vc_span4 vc_col-md-4 spg-loop-content spg-masonry switchable-view spg-'.$default_grid.' '.$el_class.'" data-view="spg-'.$default_grid.'" data-ajaxload="'.$load_ajax.'">';
		$html .= '<div id="spg-grid-gallery" class="spg-grid-gallery">';
		$html .= '<section class="spg-grid-wrap">';
        $html .= '<ul class="spg-grid spg-grids spg-clear '.$animation.'">';
        ...
        $html .= $output.'</ul>';
		$html .= '</section>';
		$html .= '</div><!-- end .spg-grid-gallery -->';
    $html .= '</div><!-- end .spg-loop-content -->';
    // pagination
    ob_start();
    ...
    ob_clean();
}//end postgrid_masonry()
add_action('wp_footer','spg_footer_script',100);
function spg_footer_script(){
