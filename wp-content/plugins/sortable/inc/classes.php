...
class sortable {
	...
	public function get_html($social_status,$include_social_media,$sort_social_media,$alphabetically,$comments,$post_type,$per_page,$show_stats,$default_view,$color,$custom){
		global $wpdb;
		$rand = rand(111111,999999); $paged=1;
		...
		if($social_status!=0){
			...
			$sortby = 'social_status';
			if(preg_match('/facebook/i',$include_social_media)){
				$dropMedia .='<li><a href="#" sort-by="social_facebook" post-type="'.$post_type.'" per-page="'.$per_page.'"><i class="fa fa-facebook"></i> '.$this->translate_arr[2].'</a></li>';
			}
			if(preg_match('/google/i',$include_social_media)){
				$dropMedia .='<li><a href="#" sort-by="social_google_plus" post-type="'.$post_type.'" per-page="'.$per_page.'"><i class="fa fa-google-plus"></i> '.$this->translate_arr[4].'</a></li>';
			}
			if(preg_match('/stumble/i',$include_social_media)){
				$dropMedia .='<li><a href="#" sort-by="social_stumbleupon" post-type="'.$post_type.'" per-page="'.$per_page.'"><i class="fa fa-stumbleupon"></i> '.$this->translate_arr[5].'</a></li>';
			}
			if(preg_match('/linked/i',$include_social_media)){
				$dropMedia .='<li><a href="#" sort-by="social_linkedin" post-type="'.$post_type.'" per-page="'.$per_page.'"><i class="fa fa-linkedin"></i> '.$this->translate_arr[6].'</a></li>';
			}
			if(preg_match('/pinterest/i',$include_social_media)){
				$dropMedia .='<li><a href="#" sort-by="social_pinterest" post-type="'.$post_type.'" per-page="'.$per_page.'"><i class="fa fa-pinterest"></i> '.$this->translate_arr[7].'</a></li>';
			}
			$dropMedia .= $this->get_other_social_media($post_type,$per_page,$include_social_media);
			$dropSocialStatus = '
			<li><a href="#" class="sort-social-media-link" sort-by="social_status" sort-social-media="'.$sort_social_media.'" post-type="'.$post_type.'" per-page="'.$per_page.'" style="border-bottom:1px solid #ddd;">'.$this->translate_arr[1].'</a>
						<ul>
						'.$dropMedia.'
						</ul>
					</li>';
		...
		<div id="sortableTop'.$rand.'" class="sortable-top" rand="'.$rand.'" >
		...
		<div id="sortableContainer'.$rand.'" class="sortable-container container-fluid '.$defaultViewClass.'">
			...
			//foreach($links as $link){
				...
				*/
				...
				$htmlArr[$sortbyid] =  '
						...
						<div class="social-share-stats"><h4>Social Share Stats</h4><strong>'.$fb.' <i class="fa fa-facebook"></i></strong>  
						 <strong>'.$gp.' <i class="fa fa-google-plus"></i></strong> <strong>'.$st.' <i class="fa fa-stumbleupon"></i></strong>  
						  <strong>'.$pin.' <i class="fa fa-pinterest"></i></strong>  <strong>'.$lin.' <i class="fa fa-linkedin"></i></strong>
							'.$sortableNewtworksHMTL.'
						</div>
