...
class DpProEventCalendar {
	...
	function getFormattedEventData($get = "", $post_id = "") {
		...
		if($get == 'date' || $get == 'book_event' || $get == 'actions' || $get == 'attendees') {
			...
			if(is_numeric($_GET['event_date']) && in_array(date('Y-m-d H:i:s', $_GET['event_date']), $valid_dates)) {
		...
		switch($get) {
			...
			case 'custom_fields':
				if(is_array($dpProEventCalendar['custom_fields_counter'])) {
					...
					foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
						...
						if($field_value != "") {
							$return .= '<div class="pec_event_page_custom_fields">';
			...
			case 'date':
				...
				if(!$event_data->tbc) {
					...
					$time = date(get_option('time_format'), strtotime($event_data->date));
					...
					if($end_time != "") {
						...
						$end_time_tmp = date(get_option('time_format'), strtotime("2000-01-01 ".$end_time.":00"));
					...
					if($show_more_dates) {
						...
						$return .= 			"<div class='pec_event_page_action_menu'>";
							...
							foreach($event_dates as $key) {
								...
								$return .= 			"<li".(substr($key->date, 0, 10) == substr($event_data->date, 0, 10) ? ' class="pec_event_page_action_menu_active"' : '').">".dpProEventCalendar_date_i18n(get_option('date_format'), strtotime($key->date))."</li>";
			...
			case 'actions':
				...
				if(is_user_logged_in()) {
					...
					if(($current_user->ID == get_post_field( 'post_author', $post_id) || current_user_can('switch_themes') ) && ($this->calendar_obj->allow_user_edit_event && $this->opts['allow_user_edit_remove'])) {
						...
						$return .= 			"<li><a href='javascript:void(0);' title='".$this->translation['TXT_EDIT_EVENT']."' data-event-id='".$post_id."' class='pec_edit_event'><i class='fa fa-pencil'></i>".$this->translation['TXT_EDIT_EVENT']."</a></li>";
					}
				}
	...
	function getBookingsCount($post_id, $date) {
		...
		$querystr = "
            ...
            FROM ".$this->table_booking."
			WHERE id_event IN(".$id_list.") AND event_date = '".$date."' AND status <> 'pending' AND status <> 'canceled_by_user' AND status <> 'canceled'
            ";
        $bookings_obj = $wpdb->get_results($querystr, OBJECT);
		...
		if(is_array($dpProEventCalendar['booking_custom_fields_counter']) && !$hide_buttons) {
			...
			foreach($dpProEventCalendar['booking_custom_fields_counter'] as $key) {
				...
				if($dpProEventCalendar['booking_custom_fields']['type'][$counter] == "checkbox") {
					...
					<input type="checkbox" class="checkbox pec_event_page_book_extra_fields '.(!$dpProEventCalendar['booking_custom_fields']['optional'][$counter] ? 'pec_required' : '').'" value="1" id="" name="pec_custom_'.$dpProEventCalendar['booking_custom_fields']['id'][$counter].'" /> '.$dpProEventCalendar['booking_custom_fields']['placeholder'][$counter].'
					</div>';
		
				} else {

					$return .= '
					<input type="text" class="dpProEventCalendar_input pec_event_page_book_extra_fields '.(!$dpProEventCalendar['booking_custom_fields']['optional'][$counter] ? 'pec_required' : '').'" value="" placeholder="'.$dpProEventCalendar['booking_custom_fields']['placeholder'][$counter].'" id="" name="pec_custom_'.$dpProEventCalendar['booking_custom_fields']['id'][$counter].'" />';
		...
		if(!$hide_buttons) {
			...
			$return .= '
				...
				<p class="pec_booking_date">
					...
					<select name="pec_event_page_book_date" id="pec_event_page_book_date"> 
					';
			...
			if(!is_array($event_dates) || count($event_dates) == 0) {
				...
				foreach($event_dates as $upcoming_date) {
					...
					if(!$upcoming_date->pec_daily_working_days && $upcoming_date->recurring_frecuency == 1 && $upcoming_date->pec_daily_every > 1 && 
						( ((strtotime($curDate) - strtotime(substr($upcoming_date->date,0,11))) / (60 * 60 * 24)) % $upcoming_date->pec_daily_every != 0 )
					) {
						continue;
					}
					...
					if($upcoming_date->recurring_frecuency == 3 && $upcoming_date->pec_monthly_every > 1 && 
						( !is_int (((date('m', strtotime($curDate)) - date('m', strtotime(substr($upcoming_date->date,0,11))))) / ($upcoming_date->pec_monthly_every)) )
					) {
						continue;
					}
					...
					if($upcoming_date->end_time_hh != "" && $upcoming_date->end_time_mm != "") {
						...
						$end_time = date(get_option('time_format'), strtotime("2000-01-01 ".$end_time.":00"));

						if($end_time == date(get_option('time_format'), strtotime($upcoming_date->date))) {
							$end_time = "";	
						}
					}
					...
					).($this->calendar_obj->show_time && !$upcoming_date->hide_time && $end_time != "" ? '-' . $end_time : '').' '.($booking_limit > 0 ? '('.$booking_available.' '.$this->translation['TXT_BOOK_TICKETS_REMAINING'].')' : '').'</option>';	
			...
			$return .= '<select name="pec_event_page_book_quantity" id="pec_event_page_book_quantity">';
	...
	function getBookingButton($post_id, $date = '', $clear = true, $show_attendees = true) {
		...
		if(
			...
			if(is_numeric($pec_booking_block_hours) && $pec_booking_block_hours > 0) {
				...
				$event_dates = $this->upcomingCalendarLayout( true, 1, '', null, null, true, false, true, false, false, '', false, $start_date_from );
				...
				if(($this->calendar_obj->date_range_start != '0000-00-00' && $this->calendar_obj->date_range_start != NULL && (strtotime($curDate) < strtotime($this->calendar_obj->date_range_start)) || ( $this->calendar_obj->date_range_end != '0000-00-00' && $this->calendar_obj->date_range_end != NULL && strtotime($curDate) > strtotime($this->calendar_obj->date_range_end))) && !$this->is_admin) {
					...
					if($this->calendar_obj->show_titles_monthly && empty($this->event_id) && !$compact) {
						...
						foreach($eventsCurrDate as $event) {
							...
							$time = date(get_option('time_format'), strtotime($event->date));
							...
							$html .= '<a class="dp_daily_event" style="'.($event_color != "" ? 'background-color:'.$event_color.';' : '').($count_monthly_title >= 3 ? 'display:none;' : '').'" data-dppec-event="'.$event->id.'" href="'.($this->calendar_obj->link_post ? dpProEventCalendar_get_permalink($event->id).(strpos(dpProEventCalendar_get_permalink($event->id), "?") === false ? "?" : "&").'event_date='.strtotime($event->date) : 'javascript:void(0);').'" target="'.$this->calendar_obj->link_post_target.'" data-dppec-date="'.$curDate.'">'.$title.'</a>';
						...
						if($count_monthly_title > 3) {
							...
							$html .= '<span class="dp_daily_event dp_daily_event_show_more"> + '.($count_monthly_title - 3).' '.$this->translation['TXT_MORE'].'</span>';
		...
		if($this->calendar_obj->daily_weekly_layout == 'schedule') {
			...
			for($i = $this->calendar_obj->limit_time_start; $i <= $this->calendar_obj->limit_time_end; $i++) {
				...
				foreach($result as $event) {
					...
					$html .= '<a class="dp_daily_event" '.($event_color != "" ? 'style="background-color:'.$event_color.'"' : '').' data-dppec-event="'.$event->id.'" href="'.($this->calendar_obj->link_post ? dpProEventCalendar_get_permalink($event->id).(strpos(dpProEventCalendar_get_permalink($event->id), "?") === false ? "?" : "&").'event_date='.strtotime($event->date) : 'javascript:void(0);').'" target="'.$this->calendar_obj->link_post_target.'" data-dppec-date="'.$curDate.'">'.$title.'</a>';
				}
				$html .= '
					</div>';
			}
		...
		for($x = 1; $x <= 7; $x++) {
			...
			$curDate = date('Y-m-d', $week_days[$x - 1]);
				...
				foreach($result as $event) {
					...
					$html_tmp = '<a class="dp_daily_event" '.($event_color != "" ? 'style="background-color:'.$event_color.'"' : '').' data-dppec-event="'.$event->id.'" href="'.($this->calendar_obj->link_post ? dpProEventCalendar_get_permalink($event->id).(strpos(dpProEventCalendar_get_permalink($event->id), "?") === false ? "?" : "&").'event_date='.strtotime($event->date) : 'javascript:void(0);').'" target="'.$this->calendar_obj->link_post_target.'" data-dppec-date="'.$curDate.'">'.$title.'</a>';
	...
	function eventsListLayout($date, $return_btn = true) {
		...
		<div class="dp_pec_columns_1 dp_pec_isotope dp_pec_date_event_wrap dp_pec_date_block_wrap">
			...
			';
			if($return_btn) {
				$html .= '<a href="javascript:void(0);" class="dp_pec_date_event_back dp_pec_btnright"><i class="fa fa-angle-double-left"></i> <span>'.$this->translation['TXT_BACK'].'</span></a>';
			}
		$html .= '
		</div>
		<div class="dp_pec_clear"></div>';	
	...
	function getSearchResults($key, $type = '') {
		...
		if($type == '') {
			...
			<div class="dp_pec_date_block_wrap dp_pec_date_event_search dp_pec_isotope">
				...
				<a href="javascript:void(0);" class="dp_pec_date_event_back dp_pec_btnright"><i class="fa fa-angle-double-left"></i> <span>'.$this->translation['TXT_BACK'].'</span></a>
			</div>
			<div class="dp_pec_clear"></div>';
		}
	...
	function singleEventLayout($result, $search = false, $selected_date = null, $show_end_date = true, $type = '', $force = false) {
		...
		foreach($result as $event) {
			...
			if($this->calendar_obj->link_post) {
				...
				if($selected_date != "") {
					$title = '<a href="'.$permalink.(strpos($permalink, "?") === false ? "?" : "&").'event_date='.strtotime($selected_date).'" target="'.$this->calendar_obj->link_post_target.'">'.$title.'</a>';	
					$permalink = $permalink.(strpos($permalink, "?") === false ? "?" : "&").'event_date='.strtotime($selected_date);
				} else {
			...
			if($this->calendar_obj->ical_active && !$search) {
				$html .= "<a class='dpProEventCalendar_feed' href='".dpProEventCalendar_plugin_url( 'includes/ical_event.php?event_id='.$event->id.'&date='.strtotime($selected_date) ) . "'><i class='fa fa-calendar-plus-o'></i>iCal</a>";
			}
				...
				$html .= '
					<div class="dp_pec_event_description">
						<div class="dp_pec_event_description_short" '.(str_word_count($event_desc) > $this->limit_description || $excerpt != "" ? 'style="display:block"' : '').'>
							<p>'.do_shortcode($event_desc_short).'</p>
							<a href="'.($permalink == "" ? 'javascript:void(0);' : $permalink).'" target="'.$this->calendar_obj->link_post_target.'" class="dp_pec_event_description_more" '.($map_id != "" ? 'onclick="setTimeout(initialize_'.$map_id.', 100);"' : '').'>'.$this->translation['TXT_MORE'].'</a>
						</div>
		...
		if($i > $pagination) {
			$html .= '<a href="javascript:void(0);" class="pec_action_btn dpProEventCalendar_load_more" data-total="'.$i.'" data-pagination="'.$pagination.'">'.$this->translation['TXT_MORE'].'</a>';
		}
		...
		if(count($events_obj) == 0) {
			...
			if(empty($order_events)) {
				...
				if(($event_counter - 1) > $pagination) {
					$html .= '<a href="javascript:void(0);" class="pec_action_btn dpProEventCalendar_load_more" data-total="'.($event_counter - 1).'" data-pagination="'.$pagination.'">'.$this->translation['TXT_MORE'].'</a>
						<div class="dp_pec_clear"></div>
					';
				}
			}
		...
		if($this->type == 'calendar') {
			...
			if(!$this->is_admin && ($this->calendar_obj->ical_active || $this->calendar_obj->rss_active || $this->calendar_obj->subscribe_active || $this->calendar_obj->show_view_buttons)) {
				$html .= '<div class="dp_pec_options_nav">';
					if($this->calendar_obj->show_view_buttons) {
						...
						$html .= '<a href="javascript:void(0);" class="dp_pec_view dp_pec_view_action '.($this->calendar_obj->view == "monthly" || $this->calendar_obj->view == "monthly-all-events" ? "active" : "").'" data-pec-view="monthly">'.$this->translation['TXT_MONTHLY'].'</a>';

						$html .= '<a href="javascript:void(0);" class="dp_pec_view dp_pec_view_action '.($this->calendar_obj->view == "weekly" ? "active" : "").'" data-pec-view="weekly">'.$this->translation['TXT_WEEKLY'].'</a>';

						$html .= '<a href="javascript:void(0);" class="dp_pec_view dp_pec_view_action '.($this->calendar_obj->view == "daily" ? "active" : "").'" data-pec-view="daily">'.$this->translation['TXT_DAILY'].'</a>';
			...
			if($this->calendar_obj->allow_user_add_event && 
				...
				$html .= '
					...
					$html .= '
						<a href="javascript:void(0);" class="dp_pec_view dp_pec_add_event pec_action_btn dp_pec_btnright"><span class="fa fa-plus-circle"></span>'.str_replace('+', '', $this->translation['TXT_ADD_EVENT']).'</a>
					...
					if(!is_user_logged_in() && !$this->calendar_obj->assign_events_admin) {
						...
						$html .= '
							<form name="dp_pec_event_form" class="add_new_event_form" enctype="multipart/form-data" method="post">
										...
										<div class="dp_pec_row dp_pec_cal_new_sub">
												...
												<div class="dp_pec_row">
													...
													if($this->calendar_obj->form_show_start_time) {
														...
														<div class="dp_pec_col6">
															...
															<select name="time_hours" class="dp_pec_new_event_time" id="" style="width:'.(dpProEventCalendar_is_ampm() ? '70' : '50').'px;">';
													...
													if($this->calendar_obj->form_show_end_time) {
														...
														<div class="dp_pec_col6">
															...
															<select name="end_time_hh" class="dp_pec_new_event_time" id="" style="width:'.(dpProEventCalendar_is_ampm() ? '70' : '50').'px;">
																<option value="">--</option>';
																for($i = 0; $i <= 23; $i++) {
																	$hour = str_pad($i, 2, "0", STR_PAD_LEFT);
																	if(dpProEventCalendar_is_ampm()) {
											...
											<div class="dp_pec_col6">
												...
												if($this->calendar_obj->form_show_color) {
													...
													<select name="color" class="pec_color_form">
												...
												if($this->calendar_obj->form_show_location) {
													...
													<select name="location" class="pec_location_form">
														<option value="">'.$this->translation['TXT_NONE'].'</option>
														 ';
												...
												if(is_array($dpProEventCalendar['custom_fields_counter'])) {
													...
													foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
														...
														if($dpProEventCalendar['custom_fields']['type'][$counter] == "checkbox") {
															...
															$html .= '
															<div class="dp_pec_wrap_checkbox">
															<input type="checkbox" class="checkbox '.(!$dpProEventCalendar['custom_fields']['optional'][$counter] ? 'pec_required' : '').'" value="1" id="" name="pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'" /> '.$dpProEventCalendar['custom_fields']['placeholder'][$counter].'
															</div>';
												
														} else {

															$html .= '
															<input type="text" class="dp_pec_new_event_text '.(!$dpProEventCalendar['custom_fields']['optional'][$counter] ? 'pec_required' : '').'" value="" placeholder="'.$dpProEventCalendar['custom_fields']['placeholder'][$counter].'" id="" name="pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'" />';
								...
								<div class="pec-add-footer">
									<a href="javascript:void(0);" data-lang-sending="'.$this->translation['TXT_SENDING'].'" class="dp_pec_view dp_pec_submit_event pec_action_btn dp_pec_btnright">'.($this->calendar_obj->publish_new_event ? $this->translation['TXT_SUBMIT'] : $this->translation['TXT_SUBMIT_FOR_REVIEW']).'</a>
								</div>
							</form>';
					}
				$html .= '
			...
			if(!$this->is_admin) {
				...
				if($this->calendar_obj->show_location_filter && empty($this->opts['location'])) {
					$html .= '<select name="pec_location" class="pec_location_list">
				...
				if($this->calendar_obj->show_references) {
					$html .= '
					<a href="javascript:void(0);" class="dp_pec_references dp_pec_btnright"><i class="fa fa-dot-circle-o"></i>'.$this->translation['TXT_COLOR_CODE'].'</a>
					<div class="dp_pec_references_div">
						<a href="javascript:void(0);" class="dp_pec_references_close"><i class="fa fa-times"></i></a>';
						$html .= '
				...
				$html .= '
					<a href="javascript:void(0);" class="dp_pec_view_all dp_pec_btnright" data-translation-list="'.$this->translation['TXT_LIST_VIEW'].'" data-translation-calendar="'.$this->translation['TXT_CALENDAR_VIEW'].'">';
				...
				if($this->calendar_obj->show_search) {
					...
					$html .= '
					<a href="javascript:void(0);" class="dp_pec_search_btn dp_pec_btnright">
						<i class="fa fa-search"></i>
					</a>';
				...
				if($this->calendar_obj->show_search) {
					...
					$html .= '
						...
						<form method="post" class="dp_pec_search_form">
							<input type="search" class="dp_pec_search" value="" placeholder="'.$this->translation['TXT_SEARCH'].'">
			...
			$html .= '
				...
				if(is_array($event_list)) {
					foreach ($event_list as $event) {
						...
						$html .= '<div class="dp_pec_countdown_event" data-countdown-tzo="'.$tzo.'" data-countdown-year="'.date("Y",$time).'" data-countdown-month="'.date("m",$time).'" data-countdown-day="'.date("d",$time).'" data-countdown-hour="'.date("H",$time).'" data-countdown-minute="'.date("i",$time).'">';
						...
						$html .= '			<li class="dp_pec_countdown_days_wrap"><span class="dp_pec_countdown_days">--</span><p class="dp_pec_countdown_days_txt">'.$this->translation['TXT_DAYS'].'</p></li>';
						$html .= '			<li class="dp_pec_countdown_hours_wrap"><span class="dp_pec_countdown_hours">--</span><p class="dp_pec_countdown_hours_txt">'.$this->translation['TXT_HOURS'].'</p></li>';
			...
			if(is_array($event_list)) {
				...
				foreach ($event_list as $obj) {
					...
					if(!is_object($unique_events[$obj->id])) {
						...
						$time = date(get_option('time_format'), strtotime($obj->date));
					...
					function pec_codeAddress(link, category, phone, address, title, image, eventdate, latlng, video) {
						...
						setTimeout(function() { 
						  ...
						  if(!jQuery(".pec_infowindow_close", infoBubble.bubble_).length) {
						  	var close = jQuery(\'<a href="javascript:void(0);" id="pec_infowindow_close" class="pec_infowindow_close"><i class="fa fa-close"></i></a>\');
							...
							<div class="dp_pec_col6">
								...
								if($this->calendar_obj->form_show_start_time) {
									...
									<div class="dp_pec_col6">
										<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_START_TIME'].'</span>
										<div class="dp_pec_clear"></div>
										<select class="dp_pec_new_event_time" name="time_hours" id="" style="width:'.(dpProEventCalendar_is_ampm() ? '70' : '50').'px;">';
											for($i = 0; $i <= 23; $i++) {
												$hour = str_pad($i, 2, "0", STR_PAD_LEFT);
												if(dpProEventCalendar_is_ampm()) {
								...
								if($this->calendar_obj->form_show_end_time) {
									...
									<div class="dp_pec_col6">
										<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_END_TIME'].'</span>
										<div class="dp_pec_clear"></div>
										<select class="dp_pec_new_event_time" name="end_time_hh" id="" style="width:'.(dpProEventCalendar_is_ampm() ? '70' : '50').'px;">
											<option value="">--</option>';
											for($i = 0; $i <= 23; $i++) {
												$hour = str_pad($i, 2, "0", STR_PAD_LEFT);
												if(dpProEventCalendar_is_ampm()) {
				...
				foreach($bookings_list as $key) {
					...
					switch($key->status){
						...
						if($this->calendar_obj->booking_cancel && $key->status != 'canceled_by_user' && $key->status != 'canceled') {
							$html .= '<a href="javascript:void(0);" class="pec_cancel_booking">'.$this->translation['TXT_CANCEL_BOOKING'].'</a>
	...
	function eventsMonthList($start_search = null, $end_search = null, $limit = 40, $keyword = '') {
		...
		if(is_array($event_list) && count($event_list) > 0) {
			...
			foreach($event_list as $event) {
				...
				if($this->calendar_obj->link_post) {
					$permalink = dpProEventCalendar_get_permalink($event->id);
					$title = '<a href="'.$permalink.(strpos($permalink, "?") === false ? "?" : "&").'event_date='.strtotime($event->date).'" target="'.$this->calendar_obj->link_post_target.'">'.$title.'</a>';	
				}
				...
				$html .= '
						<div class="dp_pec_event_description">
							<div class="dp_pec_event_description_short" '.(str_word_count($event_desc) > $this->limit_description || $excerpt != "" ? 'style="display:block"' : '').'>
								<p>'.do_shortcode($event_desc_short).'</p>
								<a href="'.($permalink == "" ? 'javascript:void(0);' : $permalink).'" target="'.$this->calendar_obj->link_post_target.'" class="dp_pec_event_description_more" '.(isset($map_id) && $map_id != "" ? 'onclick="setTimeout(initialize_'.$map_id.', 100);"' : '').'>'.$this->translation['TXT_MORE'].'</a>
							</div>
							<div class="dp_pec_event_description_full" '.(str_word_count($event_desc) > $this->limit_description || $excerpt != "" ? 'style="display:none"' : '').'>
								'.do_shortcode($event_desc);
					$html .= '
		...
		if(is_array($event_list) && count($event_list) > 0) {
			...
			foreach($event_list as $event) {
				...
				$html .= '<li class="dp_pec_grid_event dp_pec_isotope dp_pec_grid_columns_'.$this->columns.' '.($no_bg ? 'dp_pec_grid_no_img' : '').'" data-event-number="'.$event_counter.'" '.($event_counter > $pagination ? 'style="display:none;"' : '').'>';
					...
					$html .= '<a href="'.dpProEventCalendar_get_permalink($event->id).(strpos(dpProEventCalendar_get_permalink($event->id), "?") === false ? "?" : "&").'event_date='.strtotime($event->date).'" class="dp_pec_grid_link_image" target="'.$this->calendar_obj->link_post_target.'" title="" style="background-image:url('.$event_bg.');"></a>';
	...
	function getRating($eventid) {
		...
		if($rate != '' || $rate === 0) {
			...
			<ul class="dp_pec_rate '.($user_rate != "" && is_user_logged_in() ? 'dp_pec_user_rate' : '').'">
				<li>
					'.($user_rate != "" && is_user_logged_in() ? '<a href="javascript:void(0);" data-rate-val="1" data-event-id="'.$eventid.'">'.$s1.'</a>' : '<span>'.$s1.'</a>').'
				</li>
				<li>
					'.($user_rate != "" && is_user_logged_in() ? '<a href="javascript:void(0);" data-rate-val="1" data-event-id="'.$eventid.'">'.$s2.'</a>' : '<span>'.$s2.'</a>').'
				</li>
				<li>
					'.($user_rate != "" && is_user_logged_in() ? '<a href="javascript:void(0);" data-rate-val="1" data-event-id="'.$eventid.'">'.$s3.'</a>' : '<span>'.$s3.'</a>').'
				</li>
				<li>
					'.($user_rate != "" && is_user_logged_in() ? '<a href="javascript:void(0);" data-rate-val="1" data-event-id="'.$eventid.'">'.$s4.'</a>' : '<span>'.$s4.'</a>').'
				</li>
				<li>
					'.($user_rate != "" && is_user_logged_in() ? '<a href="javascript:void(0);" data-rate-val="1" data-event-id="'.$eventid.'">'.$s5.'</a>' : '<span>'.$s5.'</a>').'
				</li>
			</ul>';
		}
	...
	function getEditRemoveButtons($event) {
		...
		if(is_user_logged_in() && (($this->calendar_obj->allow_user_edit_event || $this->calendar_obj->allow_user_remove_event) && $this->opts['allow_user_edit_remove'])) {
			...
			if(($current_user->ID == get_post_field( 'post_author', $event->id) || current_user_can('switch_themes') ) && ($this->calendar_obj->allow_user_edit_event && $this->opts['allow_user_edit_remove'])) {
				...
				$edit_button .= '<a href="javascript:void(0);" title="'.$this->translation['TXT_EDIT_EVENT'].'" data-event-id="'.$event->id.'" class="pec_edit_event"><i class="fa fa-pencil-square-o"></i></a>';		
			}
			...
			if(($current_user->ID == get_post_field( 'post_author', $event->id) || current_user_can('switch_themes' )) && ($this->calendar_obj->allow_user_remove_event && $this->opts['allow_user_edit_remove'])) {
				$edit_button .= '<a href="javascript:void(0);" title="'.$this->translation['TXT_REMOVE_EVENT'].'" class="pec_remove_event"><i class="fa fa-trash"></i></a><div style="display:none;">
				<form enctype="multipart/form-data" method="post" class="add_new_event_form remove_event_form">
					...
					if(is_array($dpProEventCalendar['custom_fields_counter'])) {
						...
						foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
							if($dpProEventCalendar['custom_fields']['type'][$counter] == "checkbox") {
								...
								<input type="checkbox" class="checkbox '.(!$dpProEventCalendar['custom_fields']['optional'][$counter] ? 'pec_required' : '').'" value="1" id="" '.(is_numeric($edit) && get_post_meta($edit, 'pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter], true) ? 'checked="checked"' : '').' name="pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'" /> '.$dpProEventCalendar['custom_fields']['placeholder'][$counter].'
								</div>';
								
							} else {
								
								$html .= '
								<input type="text" class="dp_pec_new_event_text '.(!$dpProEventCalendar['custom_fields']['optional'][$counter] ? 'pec_required' : '').'" value="'.(is_numeric($edit) ? get_post_meta($edit, 'pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter], true) : ''). '" placeholder="'.$dpProEventCalendar['custom_fields']['placeholder'][$counter].'" id="" name="pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'" />';
							}
	...
	function userHasBooking($date, $event_id) {
		...
		$querystr = "
            ...
            FROM ".$this->table_booking."
			WHERE id_event = (".$id_list.") AND id_user = ".$current_user->ID." AND event_date = '".$date."' AND status <> 'pending' AND status <> 'canceled_by_user' AND status <> 'canceled'
            ";
        $bookings_obj = $wpdb->get_results($querystr, OBJECT);
