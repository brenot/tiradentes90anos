...
function dpProEventCalendar_getEvents() {
	...
	$dpProEventCalendar_class->switchCalendarTo($type);
...
function dpProEventCalendar_pro_event_calendar_init() {
  ...
  $args = array(
    ...
    'show_in_menu' => true, 
    'query_var' => true,
    'rewrite' => array( 'slug' => 'pec-organizers' ),
    'capability_type' => 'post',
    'has_archive' => false, 
    'hierarchical' => false,
	'show_in_menu' => 'dpProEventCalendar-admin',
	...
	$querystr = "
		...
		if(!empty($bookings_obj)) {
			...
			foreach($bookings_obj as $booking) {
				...
				wp_mail( $userdata->user_email, get_bloginfo('name'), apply_filters('pec_booking_email', $booking_email_template_reminder_user, $booking->id_event, $userdata->display_name, $userdata->user_email, $booking->event_date, $booking->comment, $booking->quantity, $booking->phone), $headers );
			}
	...
	foreach($calendars_obj as $key) {
		...
		if($key->sync_ical_enable && ($key->sync_ical_url != "" || $key->sync_fb_page != "")) {
			...
			add_action( 'pecsyncical'.$key->calendar_id, 'dpProEventCalendar_ical_sync', 10 ,1 );
					/*if($key->calendar_id == 1 && $_GET['pec_debug']) {
						echo $key->calendar_id;
...
function dpProEventCalendar_fetch_media($file_url, $post_id) {
	...
	if (@fclose(@fopen($file_url, "r", false, $context))) { //make sure the file actually exists
		if(!copy($file_url, ABSPATH.$artDir.$new_filename, $context))
		{
...
function dpProEventCalendar_getMoreBookings($limit = 30, $offset = '', $eventid = '') {
...
function dpProEventCalendar_footerSingle() {
	...
	if($_GET['event_date'] != "") {
		$startDate = date('Y-m-d H:i:s', $_GET['event_date']);
